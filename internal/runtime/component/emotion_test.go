...
func TestEmotionComponent_Tick_Decay(t *testing.T) {
	...
	name, ok := blackboard.Get(board, blackboard.KeyEmotionDominant)
	if !ok {
	...
	val, ok := blackboard.Get(board, blackboard.KeyEmotionDominantVal)
	if !ok {
...
func TestEmotionComponent_Tick_FearAccumulate_WithThreatMemory(t *testing.T) {
	...
	// 设置威胁记忆 → fear 应累积而非衰减
	blackboard.Set(board, blackboard.KeyMemoryThreatValue, 50.0)

	// dt=1.0 → fear: 10+20=30（累积，不衰减）
	tickable.Tick(board, 1.0)

	val, _ := blackboard.Get(board, blackboard.KeyEmotionDominantVal)
	if val != 30 {
...
func TestEmotionComponent_Tick_FearDecay_NoThreatMemory(t *testing.T) {
	...
	blackboard.Set(board, blackboard.KeyMemoryThreatValue, 0.0)
...
func TestEmotionComponent_Tick_NonFearDecays_WithThreatMemory(t *testing.T) {
	...
	name, _ := blackboard.Get(board, blackboard.KeyEmotionDominant)
